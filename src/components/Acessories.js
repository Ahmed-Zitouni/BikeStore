...
const Clicked = {background: '#ff5e57', border: '2px solid #ff5e57'}
...
useEffect(() => {
  Cart.map(list => {
    if (list.Name === Products[0].Name)
      setItems(prevState => ({ ...prevState, "1": true }));
    if (list.Name === Products[1].Name)
      setItems(prevState => ({ ...prevState, "2": true }));
    if (list.Name === Products[2].Name)
      setItems(prevState => ({ ...prevState, "3": true }));
  });
}, []);
  return (
